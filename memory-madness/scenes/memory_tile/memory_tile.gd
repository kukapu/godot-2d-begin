...
func reveal(is_visible: bool) -> void:
	frame_image.visible = is_visible
	item_image.visible = is_visible
	

func setup(image: ItemImage, frame: Texture2D) -> void:
	item_image.texture = frame
	frame_image.texture = image.get_texture()
	_item_name = image.get_name()
	reveal(false)
	

func maches_other_tile(other: MemoryTile) -> bool:
...
func _on_pressed() -> void:
	if _can_select_me and frame_image.visible == false:
